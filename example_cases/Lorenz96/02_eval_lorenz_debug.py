...
if False:
    ...
    for i in range(loaded_train_xi.shape[1]):
        ax[0,0].plot(t, pred_y[:,i,0,0])
        
        ax[0,0].plot(t, loaded_train_trajects[:,0,i], 'k-')
    ...
    e_train = np.abs((e_py - e_gt)**2)
    
    error_vector = np.sum(e_train, axis=(0, 1))
...
for i in [2,5]:
    ...
    ax[0,1].plot(t, pred_y[:,i,0,0])
    ax[0,1].plot(t, loaded_val_trajects[:,0,i], 'k-')
